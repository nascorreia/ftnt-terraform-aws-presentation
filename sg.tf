...
resource "aws_security_group" "security_groups" {
  ...
  //ex: 
  // key = external
  // value = contents of the local.security_groups.external object

  name        = each.value.tags.Name
  description = "Security group for ${each.key} SG" // "Security group for external SG"  in the first iteration
...
// Keep in mind that aws_security_group.security_groups["external"].id is a Terraform construct and "external" does not map to any property or attribute of the AWS security group itself